...
*/
int Check(const Mat &sourceImage)
{
	int t = 0;
	for (int i = 0; i < sourceImage.rows; i++)
	{
		...
		{
			int R, G, B;
			R = (int)(sourceImage.at<Vec3b>(i, j)[2]);
			G = (int)(sourceImage.at<Vec3b>(i, j)[1]);
			B = (int)(sourceImage.at<Vec3b>(i, j)[0]);
...
int main(int agrc, char *agrv[])
{
	...
	if (agrc == 4)
	{
		...
		Mat image = imread(agrv[2], CV_LOAD_IMAGE_COLOR);
		imshow("Image", image);
		Blur B;
		EdgeDetector E;
		char* d = agrv[3];
		...
		// Kiểm tra xem ảnh đầu vào là ảnh màu hay ảnh xám
		int t;
		t = Check(image);
		...
		if (strcmp(s1, agrv[1]) == 0)
		{
			...
			if (t == 1)
			{
				dest = Mat(image.rows, image.cols, CV_8UC1, Scalar(0));
				check = B.BlurImage(Image, dest, n, n, 0);
			}
			else
			{
				dest = Mat(image.rows, image.cols, CV_8UC3, Scalar(0));
				check = B.BlurImage(image, dest, n, n, 0);
			}
